 ...
 * ---- O(n^2)
 * 
 * 2.diff solution. Reduce to max subarray. 
 * Define b[i]=a[i+1]-a[i]  (0 <= i < n-1);
 * 
 * Now if we calculate sum(b[i],b[j]):
 * for k = i to k = j, assume we buy stock in day i and sell in day j + 1
 * for the scope of b, it is only from 0 to n-2
 * b[i] + b[i+1] + ... + b[j] = (a[i+1]-a[i]) + (a[i+2]-a[i+1]) + ... + (a[j+1]-a[j])
 * = a[j+1] - a[i]		(0 <= i <= j < n-1)
 * 
 * we know a[j+1] - a[i] is the difference we want to calculate, in order to make it largest,
 * we simply want to calculate the max subarray from i to j
 * 
 * @author haozheng
 * 
 */
