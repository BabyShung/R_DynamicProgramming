 ...
 * 
 * PS: substring should be consecutive, while sequence might not
 * 
 * Similar to LCSequence, it is a variant of that. Let's define a two-dimension
 * array, i is string A with length i, and j is the string B with length j,
 * c[i,j] means the LCSub length of both String A and B
 * 
 * c[i,j] = 0, ----i=j=0 or j=0 or i=0
 * 
 * c[i,j] = c[i-1,j-1] + 1, ----i,j>0 && A[i] = B[j]
 * 
 * c[i,j] = 0, ----i,j>0 && A[i] != B[j]
 * 
 * 
 * 
 * @author haozheng
